...
import cmsc420.meeshquest.part4.PRQuadtree;
import cmsc420.meeshquest.part4.CommandParser.QuadrantDistance;
...
public class CommandParser {
	...
	public CommandParser () {
		try {
	    	...
	    	spatial = new PRQuadtree(commandNode.getAttribute("localSpatialHeight"), commandNode.getAttribute("localSpatialWidth"));
	...
	private Node nearestCityToRoadXML(String err, String start, String end, City city, String id) {
		...
		if(err == null){
			...
			Element cty = null;
				...
				cty.setAttribute("localX", city.getXString());
				cty.setAttribute("localY", city.getYString());
	...
	public void rangeCities(Element commandNode){
		Point2D p = new Point2D.Double(Integer.parseInt(commandNode.getAttribute("localX")), Integer.parseInt(commandNode.getAttribute("localY")));
		...
		rootElement.appendChild(rangeCitiesXML(err, commandNode.getAttribute("localX"),commandNode.getAttribute("localY"), commandNode.getAttribute("radius"), ctys, commandNode.getAttribute("saveMap"), commandNode.getAttribute("id")));
	}
	
	public void rangeRoads(Element commandNode){
		int y = Integer.parseInt(commandNode.getAttribute("localY"));
		int x = Integer.parseInt(commandNode.getAttribute("localX"));
		...
		rootElement.appendChild(rangeRoadsXML(id, err, commandNode.getAttribute("localX"),commandNode.getAttribute("localY"), commandNode.getAttribute("radius"), rds, commandNode.getAttribute("saveMap")));
	}
	...
	private Node rangeRoadsXML(String id, String err, String xC, String yC, String rad, TreeSet<Road> roads2, String save) {
		...
		s.appendChild(com);
		Element p = results.createElement("parameters");
		Element x = results.createElement("localX");
			x.setAttribute("value", xC);
		Element y = results.createElement("localY");
	...
	public void nearestCity(Element commandNode){
		Point2D.Float p = new Point2D.Float(Integer.parseInt(commandNode.getAttribute("localX")), Integer.parseInt(commandNode.getAttribute("localY")));
		double minDist = Integer.MAX_VALUE;
		String err = null;
		City cty = null;
		...
		rootElement.appendChild(nearestCityXML(commandNode.getAttribute("id"), false, err, commandNode.getAttribute("localX"),commandNode.getAttribute("localY"), cty, null, false));
	}
	...
	public void nearestRoad(Element commandNode){
		...
		if (spatial.root.isNode().equals("white")){
			...
			for(Road road: roads){
				double dis = road.ptSegDist(Double.parseDouble(commandNode.getAttribute("localX")), Double.parseDouble(commandNode.getAttribute("localY")));
				if(dis < minDist){
					rd = road;
					minDist = dis;
				} else if (dis == minDist){
		...
		rootElement.appendChild(nearestCityXML(id,false,err, commandNode.getAttribute("localX"),commandNode.getAttribute("localY"),null,rd,true));
	}
	...
	public void nearestIsolatedCity(Element commandNode){
		...
		rootElement.appendChild(nearestCityXML(commandNode.getAttribute("id"), true, err, commandNode.getAttribute("localX"),commandNode.getAttribute("localY"), cty, null, false));
	}
	...
	public void createCity(String command, Element commandNode){
		...
		x = commandNode.getAttribute("localX");
		y = commandNode.getAttribute("localY");
		...
		//create city
		City city = new City(name, x, y, radius, color, remoteX, remoteY);
	...
	public Element createCityXML(City cty, boolean err, String type, String id){
		...
		Element xc = results.createElement("localX");
				xc.setAttribute("value",cty.getXString());
		Element yc = results.createElement("localY");
		...
		Element remotex = results.createElement("remoteX");
				remotex.setAttribute("value",cty.getremoteX());
		Element remotey = results.createElement("remoteY");
				remotey.setAttribute("value",cty.getremoteY());
					
		p.appendChild(n);
		p.appendChild(xc);
		p.appendChild(yc);
		p.appendChild(remotex);
		p.appendChild(remotey);
		p.appendChild(rad);
		p.appendChild(col);
		s.appendChild(p);
		if(!err){
	...
	private Element creatListXML(String sortBy, String id) {
		...
		if(sorted.length > 0){
			...
			for (Object c : sorted){
				...
				cty.setAttribute("localX", ((City) c).getXString());
				cty.setAttribute("localY", ((City) c).getYString());
				
				cty.setAttribute("remoteX", ((City) c).getremoteX());
				cty.setAttribute("remoteY", ((City) c).getremoteY());
				l.appendChild(cty);
			}
			o.appendChild(l);
			s.appendChild(o);
	...
	public Element deleteCityXML(String errorType, City cty, String cityName, String id){
		...
		if(errorType == null){ 
			...
			if(spatial.contains(cty) == true){ 
			 ...
			 unmapped.setAttribute("localX", cty.getXString());
			unmapped.setAttribute("localY", cty.getYString());
			unmapped.setAttribute("remoteX", cty.getremoteX());		
			unmapped.setAttribute("remoteY", cty.getremoteY());
	...
	private Node nearestCityXML(String id, Boolean iso, String err, String xC, String yC, City city, Road rd, Boolean rod) {
		...
		Element y = results.createElement("localY");
			...
			Element x2 = results.createElement("remoteX");
			x2.setAttribute("value", city.getremoteX());
		Element y2 = results.createElement("remoteY");
			y2.setAttribute("value", city.getremoteY());
			p.appendChild(x);
			p.appendChild(y);
			p.appendChild(x2);
			p.appendChild(y2);
			...
			if(err == null){
				...
				if(iso == true){
					...
					cty.setAttribute("remoteX", city.getremoteX());
					cty.setAttribute("remoteY", city.getremoteY());
					cty.setAttribute("color", city.getColor());
					cty.setAttribute("radius", city.getRadiusString());
				} else if (rod == false){
					cty = results.createElement("city");
	...
	public String getAngle(Point2D.Float p1p, Point2D.Float p2p, Point2D.Float p3p){
		    ...
		    //if(((City)p1).getName().equals("Paris") && ((City)p3).getName().equals("Athens") && ((City)p2).getName().equals("Milan"))
		    //System.out.println(((City)p1p).getName() + " " + ((City)p2p).getName() + " " + ((City)p3p).getName() + "   " + ang);
